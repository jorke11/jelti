...
select d.id,coalesce(d.invoice,'') invoice,  coalesce(s.business_name ,sta.business_name) as client,
            ...
            (select (coalesce(sum(quantity * units_sf * value * tax),0) + coalesce(sum(quantity * units_sf * value),0) + d.shipping_cost)::money from sales_detail where sale_id=sales.id and product_id is not null) totalFormated,
           (select (ceil(coalesce(sum(quantity * units_sf * value * tax),0) + coalesce(sum(quantity * units_sf * value),0)) + d.shipping_cost) from sales_detail where sale_id=sales.id and product_id is not null) total,
           (select sum(value) from briefcase where departure_id=d.id) payed,(select sum(value)::money from briefcase where departure_id=d.id) payedformated
            from departures d
            JOIN sales ON sales.departure_id=d.id
